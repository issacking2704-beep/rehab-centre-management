...
export default function StaffPage() {
  ...
  async function load() {
    try {
      ...
      const staffRows = Array.isArray(staffData.staff) ? staffData.staff as Staff[] : [];
      const attenderRows = Array.isArray(attenderData.attenders) ? attenderData.attenders as Array<{ uid: string; assignedPatientIds?: string[]; passkeyLast4?: string; passkeyCreatedAt?: string }> : [];
      const attenderMap = new Map(attenderRows.map((row) => [row.uid, row]));
      // The dedicated Patient Attenders endpoint is the authoritative assignment source.
      // Merge it into the staff table so the displayed count always reflects saved assignments.
      setStaff(staffRows.map((person) => {
        if (person.role !== "patient_attender") return person;
        const attender = attenderMap.get(person.uid);
        return attender
          ? { ...person, assignedPatientIds: Array.isArray(attender.assignedPatientIds) ? attender.assignedPatientIds : [], passkeyLast4: attender.passkeyLast4 || person.passkeyLast4, passkeyCreatedAt: attender.passkeyCreatedAt || person.passkeyCreatedAt }
          : person;
      }));
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900">
    <header className="border-b bg-white shadow-sm"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-5"><div><p className="text-sm font-semibold text-blue-600">Administration</p><h1 className="text-2xl font-bold">Doctors & Staff</h1><p className="text-sm text-slate-500">Manage staff accounts, Patient Attender passkeys and patient assignments.</p></div><button onClick={() => { setShowAdd(true); setError(""); setMessage(""); setShowPasskey(""); }} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-700">＋ Add Staff</button></div></header>
