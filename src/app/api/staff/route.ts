...
type ApiError = { category: ErrorCategory; message: string; status: number; code?: string };

function errorResponse(error: ApiError | string, status = 400) {
  if (typeof error === "string") return NextResponse.json({ success: false, error }, { status });
  return NextResponse.json({ success: false, error: error.message, category: error.category, ...(error.code ? { code: error.code } : {}) }, { status: error.status });
}
...
function classifyError(error: unknown, fallbackMessage: string): ApiError {
  ...
  if (message.includes("missing firebase admin environment variables") || message.includes("private key") || message.includes("service account") || message.includes("credential") || code === "app/invalid-credential" || code === "app/invalid-argument") {
    return { category: "firebase_admin_credentials", status: 500, code: code || "firebase_admin_credentials", message: "Firebase Admin credentials are not configured correctly on the server. Check FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, and FIREBASE_PRIVATE_KEY in the Vercel environment variables, then redeploy." };
  }
  if (message.includes("missing authentication token") || message.includes("invalid authentication header") || code === "auth/id-token-expired" || code === "auth/id-token-revoked" || code === "auth/argument-error" || code === "auth/invalid-id-token") {
    return { category: "authentication", status: 401, code: code || "authentication_error", message: code === "auth/id-token-expired" ? "Your Firebase login session has expired. Please sign in again." : code === "auth/id-token-revoked" ? "Your Firebase login session was revoked. Please sign in again." : "Authentication failed. Please sign in again and retry." };
  }
  if (message.includes("permission") || code === "permission-denied" || code === "7" || code === "auth/insufficient-permission") {
    return { category: "permissions", status: 403, code: code || "permission_denied", message: "You do not have permission to manage staff. Your Firestore/server account role must be super_admin, admin, or sub_admin." };
  }
  if (code === "5" || code === "not-found" || code === "14" || code === "unavailable" || message.includes("firestore") || message.includes("failed to connect") || message.includes("could not reach")) {
    return { category: "firestore", status: 500, code: code || "firestore_error", message: "Firestore could not complete the staff request. Check that the Firebase Admin service account has access to Firestore and that the Firestore database is enabled." };
  }
  return { category: "server", status: 500, code: code || "internal_server_error", message: fallbackMessage };
}
...
async function requireStaffManager(request: NextRequest) {
  ...
  if (role !== "super_admin" && role !== "admin" && role !== "sub_admin") throw new Error("You do not have permission to manage staff.");
...
export async function GET(request: NextRequest) {
  try {
    ...
    return errorResponse(classifyError(error, "Failed to load staff from the server."));
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const assignedPatientIds: string[] = Array.isArray(body.assignedPatientIds) ? body.assignedPatientIds.map((id: unknown) => String(id).trim()).filter(Boolean) : [];
    ...
    const cleanAssignedPatientIds = [...new Set(assignedPatientIds)];
    if (role === "patient_attender" && cleanAssignedPatientIds.length) {
      const checks = await Promise.all(cleanAssignedPatientIds.map((id: string) => adminDb.collection("patients").doc(String(id)).get()));
      const valid = checks.filter((patientDoc) => patientDoc.exists && patientDoc.data()?.isDeleted !== true).map((patientDoc) => patientDoc.id);
      cleanAssignedPatientIds.splice(0, cleanAssignedPatientIds.length, ...valid);
    }

    const authData: { displayName: string; disabled: boolean; email?: string; password?: string } = { displayName: name, disabled: false };
    ...
    let passkeyHash = "";
    try {
      const userData: Record<string, any> = { uid: userRecord.uid, name, email, phone, role, active: true, assignedPatientIds: role === "patient_attender" ? cleanAssignedPatientIds : [], createdAt: new Date() };
      if (role === "patient_attender") {
        ...
        await adminDb.collection(PATIENT_ATTENDER_PASSKEY_COLLECTION).doc(String(passkeyHash)).set({ uid: userRecord.uid, active: true, createdAt: new Date() });
      }
      ...
      if (passkeyHash) { try { await adminDb.collection(PATIENT_ATTENDER_PASSKEY_COLLECTION).doc(String(passkeyHash)).delete(); } catch {} }
    ...
    const code = getErrorCode(error);
    if (code === "auth/email-already-exists") return errorResponse({ category: "validation", status: 409, code, message: "An account with this email already exists." });
    return errorResponse(classifyError(error, "Failed to create the staff account on the server."));
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return errorResponse(classifyError(error, "Failed to update the staff account on the server."));
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return errorResponse(classifyError(error, "Failed to delete the staff account on the server."));
  }
}
